                    ...
                    <form action="{{ $education->id ? route('education.update', $education->id) : route('education.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 relative z-10">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                                @if($education->logo)
                                    <img src="{{ Str::startsWith($education->logo, 'http') || Str::startsWith($education->logo, 'Screenshot') ? '/' . $education->logo : Storage::url($education->logo) }}" alt="Logo" class="w-24 h-24 object-cover rounded-xl border border-[#4b3621]/10 dark:border-white/10 shrink-0 shadow-md">
                                @endif
